...
-type operation() :: {create_table, binary(), [#kura_column{}]}
                   | {drop_table, binary()}
                   | {alter_table, binary(), [alter_op()]}.
-type alter_op() :: {add_column, #kura_column{}}
                  | {drop_column, atom()}
...
diff(DbState, DesiredState) ->
    ...
    {CreateUp, CreateDown} = lists:foldl(fun(Table, {UpAcc, DownAcc}) ->
        Cols = maps:get(Table, DesiredState),
        {UpAcc ++ [{create_table, Table, Cols}],
         DownAcc ++ [{drop_table, Table}]}
    end, {[], []}, lists:sort(NewTables)),

    %% Existing tables: diff columns
    ExistingTables = maps:keys(DesiredState) -- NewTables,
    {AlterUp, AlterDown} = lists:foldl(fun(Table, {UpAcc, DownAcc}) ->
...
diff_columns(DbCols, DesiredCols) ->
    ...
    {ModUp, ModDown} = lists:foldl(fun(Name, {MU, MD}) ->
