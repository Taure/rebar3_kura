...
print_next_steps(AppName, ModName) ->
    rebar_api:info(
        ...
        "--- Next steps ---~n"
        "~n"
        "1. Add a backend to rebar.config deps (kura 2.x split the backend~n"
        "   into separate packages):~n"
        "~n"
        "   {kura_postgres, {git, \"https://github.com/Taure/kura_postgres.git\",~n"
        "                    {tag, \"v0.4.2\"}}}~n"
        "~n"
        "   List it in your .app.src `applications` (after kura), and add~n"
        "   pgo before kura so the pool is up at startup.~n"
        "~n"
        "2. Add the provider hook to rebar.config:~n"
        "~n"
        "   {provider_hooks, [~n"
        "       {pre, [{compile, {kura, compile}}]}~n"
        "   ]}.~n"
        "~n"
        "3. Add database config to config/sys.config:~n"
        "~n"
        "   [{kura, [~n"
        "       {repos, #{~n"
        "           ~s => #{~n"
        "               backend   => kura_backend_postgres,~n"
        "               host      => \"localhost\",~n"
        "               port      => 5432,~n"
        "               database  => \"~s_dev\",~n"
        "               user      => \"postgres\",~n"
        ...
        "   ]}].~n"
        "~n"
        "   The {repos, #{...}} map is the kura 2.x form - single-repo today,~n"
        "   forward-compatible if you add a second backend later (e.g. SQLite~n"
        "   for analytics).~n"
        "~n"
        "4. Run migrations on app startup (kura starts the pool for you):~n"
        "~n"
        "   kura_migrator:migrate(~s),~n"
        "~n"
        "5. Create a schema in src/schemas/:~n"
        "~n"
        "   -module(my_schema).~n"
        "   -behaviour(kura_schema).~n"
        "   -include_lib(\"kura/include/kura.hrl\").~n"
        "   -export([table/0, fields/0]).~n"
        "~n"
        "   table() -> ~~\"my_table\".~n"
        "   fields() -> [~n"
        "       #kura_field{name = id, type = id, primary_key = true},~n"
        "       #kura_field{name = name, type = string}~n"
        "   ].~n"
        "~n"
        "6. Run `rebar3 compile` — migrations will be auto-generated.~n",
        [ModName, AppName, ModName]
    ).
